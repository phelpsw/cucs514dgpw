 ...
 * CS514 Spring 2007
 * Cornell University
 * Homework #2 Implementation (Type Definitions)
 * */
...
namespace CS514_HW2
{
    ...
    /* Transmission Type for periodic messages
     * */
    ...
    /* Transmission Type for state message broadcasts
     * */
    ...
    /* Lock type to represent a locked message part
     * */

    [QS.Fx.Reflection.ValueClass("3`1", "Lock")]
    public sealed class Lock
    {
        public Lock(int owner,MessageId id,int startC,int endC,int time)
        {
            this.id = id;
            this.startC = startC;
            this.endC = endC;
        ...
        public string toString() {
            return id.toString() + ":" + statrC.ToString() + ":" + endC.ToString() + ":" + time.ToString();
        }
        ...
        [XmlAttribute]
        public int owner; //time to maintain lock for (in seconds)
    ...
    public class LockPosComparer : System.Collections.Generic.IComparer<Lock>
    {

        // Calls CaseInsensitiveComparer.Compare with the parameters reversed.
        int System.Collections.Generic.IComparer<Lock>.Compare(Lock x, Lock y)
        {
            return x.startC - y.startC; 
        }
    ...
    /* Object type to reprsent a universal message idetnfier (for locks or messages)
     * */
